...
public class CompanyDAO implements DAO<Company> {
	...
	public Page<Company> findPerPageByName(int page, int resultPerPage, String search) throws Exception {
		...
		try (Session session = sessionFactory.getCurrentSession()) {
			session.beginTransaction();
			companies.setResults(
					session.createQuery(COMPANIES_BY_NAME, Company.class).setFirstResult(page * resultPerPage)
							.setMaxResults(resultPerPage).setParameter("search", allSearch).getResultList());
			companies.setCurrentPage(page);
		} catch (IllegalArgumentException e) {
			throw new InvalidCompanyException(ExceptionMessage.BAD_ACCESS.getMessage());
		}
	...
	/**
	 * Permet de récupérer la liste des computers par page avec un company_id
	 * spécifique.
	 * 
	 * @param id
	 *            id à rechercher
